...
class x2Loader(Dataset):
    def __init__(self, para, training=True):
        ...
        # self.interp_list_pob = [0.7,0.3] if training else [1]
#######################################################
        if training:
            # 只用 x4，不做混合倍率
            self.interp_ratio_list = [2]
            self.interp_list_pob = [1.0]
        else:
            # 验证 / 测试阶段保持从 config 读取
            self.interp_ratio_list = [para[self.key]['interp_ratio']]
            self.interp_list_pob = [1]
#######################################################
